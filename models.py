...
class SDTR_cub(VisionTransformer):
    def __init__(self, *args, **kwargs):
        ...
        self.AutomaticWeightedLoss = AutomaticWeightedLoss(num=5)
    ...
    def forward(self, x, return_train_cam=False, label=None):
        ...
        if self.training:
            if return_train_cam:
                ...
                return x_logits, attn_logits, cams
            ...
            cam_raw = attn_weights.sum(1)[:, 0, 1:].reshape([n, h, w])
    ...
    def get_loss(self, x_logits, attn_logits, targets, loss_clip):
        loss_x = self.loss(x_logits, targets)
        loss_att = self.loss(attn_logits, targets)
        loss_dnc = self.loss(self.dnc_logits, targets)
        ...
        loss_area = torch.sum(self.attn_map) / 196.0
...
class SDTR_imnet(VisionTransformer):
    ...
    def get_loss(self, x_logits, attn_logits, targets, loss_clip=-1):
        loss_x = self.loss(x_logits, targets)
        loss_att = self.loss(attn_logits, targets)
        loss_dnc = self.loss(self.dnc_logits, targets)
        pre_map_label = self.attn_map
