...
def train_one_epoch_our(
    ...
    for samples, targets, paths in metric_logger.log_every(
        ...
        with torch.cuda.amp.autocast():
            x_logits, attn_logits, object_map = model(
                samples, return_train_cam=True, label=targets
            )

            object_map_224 = F.interpolate(
                object_map,
                (224, 224),
                mode="bilinear",
                align_corners=True,
            ...
            fg_224_eval = object_map_224 * samples  # [32, 3, 224, 224]
            bg_224_eval = (1.0 - object_map_224) * samples  # [32, 3, 224, 224]

            fg_clip_sim = clip_util.clip_forward(
                clip_model, fg_224_eval, targets, dname=args.data_set, type="fg"
            )  # [32, 1, 1]
            bg_clip_sim = clip_util.clip_forward(
                clip_model, bg_224_eval, targets, dname=args.data_set, type="fg"
            )  # [32, 1, 1]
            
            loss_clip = clip_util.ClipLoss()(fg_clip_sim, bg_clip_sim)

            (loss, learn_weights, loss_list) = model.module.get_loss(
                x_logits, attn_logits, targets, loss_clip
            )
            if args.data_set == "CUB":
                loss = loss + 2.4 * loss_clip
            else:
                loss = loss + 0.01 * loss_clip

        loss_x, loss_att, loss_area, loss_map, loss_clip, loss_dnc = loss_list
        ...
        metric_logger.update(loss_att=loss_att.item())
        metric_logger.update(loss_area=loss_area.item())
        metric_logger.update(loss_map=loss_map.item())
        metric_logger.update(loss_clip=loss_clip.item())
        metric_logger.update(loss_dnc=loss_dnc.item())
...
def evaluate(data_loader, model, device, args=None, threshold_loc=-1):
    ...
    for images, target, paths, bboxes in metric_logger.log_every(
        ...
        # compute output
        with torch.cuda.amp.autocast():
            output, cams = model(images, return_cam=True)
        ...
        if threshold_loc != -1:
            ...
            for _b in range(images.shape[0]):
                ...
                cam_map_ = cams[_b, logits[_b][0], :, :].view(14, 14, 1)
                cam_map_ = cam_map_.detach().cpu().numpy()
                cam_map_ = utils.resize_cam(cam_map_, size=(224, 224))
                estimated_bbox = utils.get_bboxes(cam_map_, cam_thr=threshold_loc)
...
def evaluate_our(data_loader, model, device, args=None, threshold_loc=-1):
    ...
    for images, target, paths, bboxes in metric_logger.log_every(
        ...
        # compute output
        with torch.cuda.amp.autocast():
            (output, cams, _, _, _) = model(images)
